    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="md:hidden fixed top-0 inset-x-0 z-30 flex items-center justify-between border-b border-border bg-card px-4 h-14">
        ...
        <div className="flex gap-1">
          {(['overview','applications','users'] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={`px-2 py-1 rounded text-xs font-medium ${activeTab === t ? 'bg-primary text-primary-foreground' : 'text-muted-foreground'}`}
            >
              {t === 'overview' ? '⊞' : t === 'applications' ? '🏥' : '👥'}
            </button>
          ))}
        </div>
      </div>
